...
const Home = ({ setIsLoggedin, setIsLoading }) => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      const response = await fetch(
        `${serverUrl}/algorithm?userID=${localStorage.getItem("user_id")}`,
        {
          method: "GET",
          headers: {
  ...
  function updateSight() {
    if (sights.length <= 3 && !isFetching) fetchSights();

    if (sights.length <= 1) setIsLoading(true);
  ...
  const CardHolder = ({
    sight,
    firstCard,
    swipeFuncs: { release, lift, move },
    cardClickHandler,
  }) => {
    return (
      <div
        className="flex flex-col h-[calc(100%-10%-1.5rem)]"
      ...
      >
        <Card
          ...
          itemData={getItemData(sight)}
          arrowClickHandler={cardClickHandler}
        />
        <Buttons
          disable={firstCard}
          handleLikeClick={() => sendSwipeMessage(sight.sight_id, true)}
          handleDislikeClick={() => sendSwipeMessage(sight.sight_id, false)}
        />
      </div>
    );
  };
    ...
    <>
      <div className="h-full w-full bg-white">
        <div className="bg-white w-full absolute overflow-hidden h-[calc(100%-var(--navbar-height))]">
          ...
          <div
            id="testtestss"
            className="bg-white flex flex-col h-[calc(100%-10%-1.5rem)]"
          >
            <Card
              mode={""}
              currentImage={currentImage}
              setCurrentImage={setCurrentImage}
              itemData={getItemData(sights[1])}
              arrowClickHandler={cardClickHandler}
            />
            <Buttons />
          </div>
        ...
        {sights
          ...
          .map((sight) => {
            ...
            return (
              <div
                key={sight.sight_id}
                id={isFirst ? "upperCard" : ""}
                ref={isFirst ? topCardRef : undefined}
              ...
              >
                ...
                <CardHolder
                  sight={sight}
                  firstCard={sights.indexOf(sight) == 0}
                  swipeFuncs={{ release, lift, move }}
...
const Buttons = ({ handleLikeClick, handleDislikeClick, disable }) => {
  return (
    <div
      id={disable ? "disable2" : ""}
      className="h-[15%] flex-col justify-center flex p-5"
    >
